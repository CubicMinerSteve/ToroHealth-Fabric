...
public class BarDisplay {
	...
	public void draw(MatrixStack matrix, LivingEntity entity) {
		...
		DrawableHelper.drawTextWithShadow(matrix, mc.textRenderer, name, xOffset, (int) 2, 16777215);
	...
	private void renderArmorIcon(MatrixStack matrix, int x, int y) {
		RenderSystem.setShaderTexture(0, ICON_TEXTURES);
		DrawableHelper.drawTexture(matrix, x, y, 34, 9, 9, 9);
	}

	private void renderHeartIcon(MatrixStack matrix, int x, int y) {
		RenderSystem.setShaderTexture(0, ICON_TEXTURES);
		DrawableHelper.drawTexture(matrix, x, y, 16 + 36, 0, 9, 9);
	}
}
