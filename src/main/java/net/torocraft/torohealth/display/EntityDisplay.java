...
public class EntityDisplay {
	...
	public void draw(DrawContext drawContext, float scale) {
		if (entity != null) {
			try {
				drawEntity(drawContext.getMatrices(), (int) xOffset, (int) yOffset, entityScale, -80, -20, entity, scale);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static void drawEntity(MatrixStack matrixStack2, int x, int y, int size, float mouseX, float mouseY, LivingEntity entity, float scale) {
		...
		matrixStack2.multiply(quaternion.mul((Quaternionfc)quaternion2));
