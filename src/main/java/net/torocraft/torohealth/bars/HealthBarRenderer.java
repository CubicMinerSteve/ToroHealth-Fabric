...
import net.torocraft.torohealth.mixin.DrawContextAccessor;
...
public class HealthBarRenderer {
	...
	public static void render(MatrixStack matrix, LivingEntity entity, double x, double y, float width,
			boolean inWorld) {
	...
	public static void drawDamageNumber(MatrixStack matrix, int dmg, double x, double y, float width) {
		...
		int textColor = dmg < 0 ? ToroHealth.CONFIG.particle.healColor : ToroHealth.CONFIG.particle.damageColor;

		Immediate vertexConsumerProvider = VertexConsumerProvider.immediate(Tessellator.getInstance().getBuffer());

		// Since the new DrawContext gives a brand new MatrixStack, we need to set it manually.
		DrawContext drawContext = new DrawContext(client, vertexConsumerProvider);
		((DrawContextAccessor) drawContext).setMatrices(matrix);
		drawContext.drawTextWithShadow(textRenderer, s, (int) (x + (width / 2) - sw), (int) y + 10, textColor);
	}
